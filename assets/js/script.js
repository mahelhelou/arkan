...
(function () {
    ...
    // WP: logo paths are injected from PHP (wp_localize_script) instead of being
    // hardcoded to images/logo.png, so a Customizer logo is respected.
    var arkanLogos = (typeof arkanVars === 'object' && arkanVars) ? arkanVars : {};
    wind.on("scroll",function () {
        ...
        if(bodyScroll > 100){
            ...
            if (arkanLogos.logoScroll) { logo.attr('src', arkanLogos.logoScroll); }
        }else{
            navbar.removeClass("nav-scroll");
            if (arkanLogos.logo) { logo.attr('src', arkanLogos.logo); }
        }
    });
    ...
    // Contact Form
    // WP: forms are rendered by Contact Form 7, which ships its own AJAX handler
    // and uses .wpcf7-form. This legacy handler is kept only for custom markup
    // that still carries the .contact__form class, and is skipped otherwise.
    var form = $('.contact__form').not('.wpcf7-form'),
    ...
    //  Scroll back to top
    // WP: the ring can be switched off in Theme Options, so guard against a
    // missing element instead of throwing and killing everything below.
    ...
    // WP: the WordPress admin bar pushes content down 32px (46px on mobile);
    // nudge the fixed navbar so it does not sit underneath it.
    if ($('body').hasClass('admin-bar')) {
        $('.navbar').css('margin-top', (window.innerWidth > 782 ? 32 : 46) + 'px');
    }
